...
_Concept:_ We rely 100% on the existing documentation (`task.md`, `ADRs`, `README`).
_Workflow:_ The new agent scans the file system, reads `task.md`, and figures it out.
...
_Concept:_ We create a dedicated file `doc/HANDOVER.md` (or `MINDSET.md`).
_Content:_ A concise summary specifically for the _next_ Agent. "Hello, we are in Phase 5. We just decided X. Do NOT change Y. Your next task is Z."
_Workflow:_ New Agent is told: "Read `doc/HANDOVER.md` first."
...
_Concept:_ I generate a text block _now_. You copy it. You paste it into the new chat prompt.
_Workflow:_ "Hi, I am continuing project X. Here is the summary: [Paste Block]."

- **Pros:**
  - **Flexible:** You can edit it before pasting.
  - **Immediate:** The agent knows context _before_ it even looks at files.
- **Cons:**
